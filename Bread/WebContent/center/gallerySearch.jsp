...
// 검색어를 포함하는 게시판 글가져오기
if(count!=0){
	galleryList=gdao.getGalleryList(startRow, pageSize, search);
}
...
<tr><th class="tno">No.</th>
    ...
    for(int i=0;i<galleryList.size();i++){
    	...
    	%>
<tr onclick="location.href='gcontent.jsp?num=<%=gb.getNum()%>&pageNum=<%=pageNum%>'" class="img" id="img"><td><%=num-- %></td>
  ...
  //한화면에 보여줄 페이지 개수 설정
   ...
   // 한화면에 보여줄 시작페이지번호 구하기   -  /  *  +
	...
	if(startPage > pageBlock){
		%><a href="gallerySearch.jsp?pageNum=<%=startPage-pageBlock%>&search=<%=search%>">Prev</a><%
	}
	// 1 2 3 4 ...10
	...
	if(endPage < pageCount){
		%><a href="gallerySearch.jsp?pageNum=<%=startPage+pageBlock%>&search=<%=search%>">Next</a><%
	}
}
%>
</div>
